...
def Notifier(type: int, title: str, msg: str):
    """
    异步弹出通知的方法
    """
    Notice = ToastNotifier()
    if type == 0:  # 成功
        Notice.show_toast(title=title, msg=msg, icon_path="./ico/Check.ico", duration=15, threaded=True)
    elif type == 1:  # 失败
        Notice.show_toast(title=title, msg=msg, icon_path="./ico/Cross.ico", duration=10, threaded=True)
    elif type == 2:  # 其他
        Notice.show_toast(title=title, msg=msg, icon_path="./ico/Tips.ico", duration=10, threaded=True)
    else:  # 其他（阻塞线程版）
        Notice.show_toast(title=title, msg=msg, icon_path="./ico/Tips.ico", duration=5, threaded=False)
    ...
    def __init__(self):
        with httpx.Client(headers=self.header) as client:
            try:
                ...
                Notifier(1, "无法访问登录地址", "请检查是否连接到校园网！")
            else:
                ...
                self.URLs[
                    "check"] = f"https://p.njupt.edu.cn:802/eportal/portal/online_list?callback=dr1002&user_account=&user_password=&wlan_user_mac=000000000000&wlan_user_ip={ip_dec}&curr_user_ip={ip_dec}"
                Notifier(3, "登录信息初始化成功！", "")
    ...
    def __OnLogin(self):
        ...
        with httpx.Client(headers=self.header, http2=False) as client:
            try:
                ...
                res = json.loads(re.search(R"\((.*?)\);", r.text).group(1))
                if int(res["result"]) == 1:
                    Notifier(0, "登录成功", res["msg"])
                else:
                    Notifier(1, "登录失败", res["msg"])
